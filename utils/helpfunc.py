...
class PlotLogger(object):
    def __init__(self, params=['loss'], nrows=None, ncols=1, axline=None):
        self.logger = OrderedDict({param: [] for param in params})
        self.ncols = ncols
        if nrows is None:
            self.nrows = len(self.logger)
        else:
            self.nrows = nrows
        self.axlines = axline
        if axline is not None:
            assert len(axline) == len(self.logger)

        assert self.nrows * self.ncols >= len(self.logger)
    ...
    def save(self, file, **kwargs):
        fig, axes = plt.subplots(nrows=self.nrows, ncols=self.ncols, figsize=(8, 6))
        fig.tight_layout()
        if self.axlines is None:
            for ax, (key, value) in zip(axes.flatten(), self.logger.items()):
                ax.plot(value, 'o-', label=key, linewidth=1, markersize=2)
                ax.legend(loc='lower right', shadow=True, fontsize='medium')
                ax.grid(b=True, which='major', color='gray', linestyle='-', alpha=0.1)
                ax.grid(b=True, which='minor', color='gray', linestyle='-', alpha=0.1)
                ax.minorticks_on()
        else:
            for ax, (key, value), axline in zip(axes.flatten(), self.logger.items(), self.axlines):
                ax.axhline(y=axline, c="r", ls="--", lw=1)
                ax.plot(value, 'o-', label=key, linewidth=1, markersize=2)
    ...
    def synchronize_between_processes(self):
        if not is_dist_avail_and_initialized():
            return
        t = []
        for value in self.logger.values():
            t.append(value)
        t = torch.tensor(t, device='cuda').float()
        dist.barrier(async_op=False)
        dist.all_reduce(t)
        t = t.tolist()
        for i, key in enumerate(self.logger.keys()):
            self.logger[key] = i
