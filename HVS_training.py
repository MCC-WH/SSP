...
def main(args):
    ...
    args.directory = os.path.join(get_checkpoint_root(), 'GLDv2-{}-HVS'.format(args.network))
    ...
    model = HVS(backbone=backbone, reduction_dim=2048, classifier_num=args.classifier_num).to(device)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for idx, (images, targets) in enumerate(metric_logger.log_every(train_loader, print_freq, header)):
            ...
            loss, logits, loss_old, logits_old = model(images.to(device, non_blocking=True), targets)
            total_loss = loss + loss_old
        ...
        with torch.no_grad():
            ...
            for idx, (inputs, labels) in enumerate(val_metric_logger.log_every(val_loader, print_freq, '>> Val Epoch: [{}]'.format(epoch))):
                ...
                loss, logits, loss_old, logits_old = model(inputs, labels)
